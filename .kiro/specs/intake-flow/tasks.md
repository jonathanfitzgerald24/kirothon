...
- [ ] 5.1 Create `server/src/services/structureAnalyzer.ts` with a `StructureAnalyzer` class
- [ ] 5.2 Implement Drive traversal: use `drive.files.list` with pagination and parallel batch requests (batch size 100), collect folder hierarchy, file names, types, sizes, Drive File IDs, last modified dates
- [ ] 5.3 Build in-memory tree from flat file list using `parents` field; handle permission errors (403/404) by skipping and logging inaccessible paths
...
- [ ] 6.5 Create `POST /api/v1/architecture/propose` route (Admin only): trigger proposal generation, store proposals in memory/cache keyed by club ID
...
- [ ] 7.8 Create `GET /api/v1/architecture/versions` route (Admin only): return last 10 architecture versions
- [ ] 7.9 Implement rollback in `ArchitectureService`: restore a previous version's tree snapshot, apply changes to Drive, update Metadata Store
...
- [ ] 14.1 Create `server/src/services/auditService.ts` with an `AuditService` class and `logAction(clubId, userId, action, resourceType, resourceId, details)` method
...
- [ ] 19.7 Create `UploadFlow` component: file picker with drag-and-drop zone, optional upload note input (280 char limit), progress bar
...
- [ ] 19.14 Create `ArchitectureEditor` component: drag-and-drop tree editor, inline rename with Smart Folder Naming, draft mode indicator, "Activate" button, version history sidebar, rollback option, per-folder access control settings
- [ ] 19.15 Create `UserManagement` component: user list, invite form (email + role), role change dropdown, remove user button, last admin protection UI feedback
