...
import { loadAccessibleCategories, checkCategoryAccess } from '../middleware/filterByAccess'
...
portalRouter.use(loadAccessibleCategories)

// GET /api/v1/portal/tree — folder tree filtered by access (8.2)
portalRouter.get('/tree', async (req: Request, res: Response): Promise<void> => {
  const user = req.user as { clubId: string; lastLoginAt: Date | null }
  try {
...
portalRouter.get('/folder/:categoryId', async (req: Request, res: Response): Promise<void> => {
  const user = req.user as { clubId: string; lastLoginAt: Date | null }
  const { categoryId } = req.params

  if (!checkCategoryAccess(categoryId, req, res)) return
...
portalRouter.get('/file/:fileId', async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (file.categoryId && !checkCategoryAccess(file.categoryId, req, res)) return
...
portalRouter.get('/file/:fileId/preview', async (req: Request, res: Response): Promise<void> => {
...
portalRouter.get('/file/:fileId/download', async (req: Request, res: Response): Promise<void> => {
  const user = req.user as { clubId: string }
  const { fileId } = req.params
  try {
    const file = await prisma.fileMeta.findUnique({ where: { id: fileId } })
...
portalRouter.post('/files/download', async (req: Request, res: Response): Promise<void> => {
  const user = req.user as { clubId: string }
  ...
  try {
    const files = await prisma.fileMeta.findMany({ where: { id: { in: fileIds }, clubId: user.clubId } })
    ...
    for (const file of accessible) {
      try {
        const response = await drive.files.get({ fileId: file.driveFileId, alt: 'media' }, { responseType: 'stream' })
        archive.append(response.data as unknown as Readable, { name: file.name })
      } catch {
...
portalRouter.get('/timeline', async (req: Request, res: Response): Promise<void> => {
  const user = req.user as { clubId: string }
  const { folder, tag } = req.query as { folder?: string; tag?: string }
  ...
  try {
    ...
    for (const file of files) {
      if (file.categoryId && !req.accessibleCategoryIds?.has(file.categoryId)) continue
...
portalRouter.get('/folder/:categoryId/hover', async (req: Request, res: Response): Promise<void> => {
  const user = req.user as { clubId: string }
  const { categoryId } = req.params
  ...
  try {
    const [recentFiles, totalCount] = await Promise.all([
      ...
      prisma.fileMeta.count({ where: { categoryId, clubId: user.clubId } }),
    ])
