...
export type RoutingDecision =
  | { type: 'auto_placed'; categoryId: string; explanation: string }
  | { type: 'needs_selection'; options: RoutingScore[] }
  | { type: 'no_match'; suggestedCategory?: { name: string; parentId?: string; rationale: string } }

export interface RoutingResult {
  fileId: string
  fileName: string
  decision: RoutingDecision
  duplicateWarning?: { existingFileId: string; existingFileName: string }
...
export class UploadRouter {
  ...
  async checkDuplicate(fileName: string, sizeBytes: bigint, categoryId?: string): Promise<{ existingFileId: string; existingFileName: string } | null> {
    const sizeMin = BigInt(Math.floor(Number(sizeBytes) * 0.95))
    const sizeMax = BigInt(Math.ceil(Number(sizeBytes) * 1.05))

    const existing = await prisma.fileMeta.findFirst({
      where: {
        clubId: this.clubId,
        name: fileName,
  ...
  async suggestRename(fileName: string): Promise<string | null> {
    const noisePatterns = /(?:final|FINAL|copy\s*of|untitled|v\d+|_v\d+|\(\d+\))/i
    if (!noisePatterns.test(fileName)) return null

    try {
      const response = await geminiClient.generateContent('gemini-1.5-flash',
...
Suggest a clean canonical filename. Return ONLY the suggested filename, nothing else.`)
  ...
  async scoreCategories(fileName: string, mimeType: string, sizeBytes: bigint): Promise<RoutingScore[]> {
...
Return ONLY valid JSON.`
      ...
      const parsed = JSON.parse(cleaned) as Array<{ categoryName: string; score: number; explanation: string }>
  ...
  async suggestNewCategory(fileName: string, mimeType: string): Promise<{ name: string; parentId?: string; rationale: string } | null> {
    const categories = await prisma.category.findMany({
      where: { clubId: this.clubId },
...
Return ONLY valid JSON.`
      ...
      const response = await geminiClient.generateContent('gemini-1.5-flash', prompt)
      const cleaned = response.replace(/```json\n?/g, '').replace(/```\n?/g, '').trim()
      return JSON.parse(cleaned) as { name: string; rationale: string }
    } catch {
      return null
    }
  }
  ...
  async routeFile(
    ...
    if (decision.type === 'auto_placed') {
      ...
      if (suggestion) {
        (decision as RoutingDecision & { type: 'no_match' }).suggestedCategory = suggestion
      }
      // Store as pending
      const pendingFile = await prisma.fileMeta.create({
        data: {
          clubId: this.clubId,
