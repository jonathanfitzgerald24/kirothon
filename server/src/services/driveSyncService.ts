...
export class DriveSyncService {
  ...
  async registerWebhook(): Promise<{ channelId: string; expiry: Date }> {
  ...
  async processChanges(): Promise<{ processed: number; drifts: number }> {
    const club = await prisma.club.findUnique({ where: { id: this.clubId } })
    if (!club?.drivePageToken) throw new Error('No page token — register webhook first')
    ...
    while (hasMore) {
      ...
      for (const change of changes) {
        ...
        if (change.removed || file?.trashed) {
          // File deleted
          const existing = await prisma.fileMeta.findFirst({ where: { driveFileId: fileId, clubId: this.clubId } })
          if (existing) {
            await prisma.tag.deleteMany({ where: { fileId: existing.id } })
            await prisma.fileMeta.delete({ where: { id: existing.id } })
          }

          // Check if it's a folder
          const existingCategory = await prisma.category.findFirst({ where: { driveFolderId: fileId, clubId: this.clubId } })
          if (existingCategory) {
            await prisma.structuralDrift.create({
              data: {
                clubId: this.clubId,
          ...
          if (isFolder) {
            ...
            const existing = await prisma.category.findFirst({ where: { driveFolderId: fileId, clubId: this.clubId } })
            if (!existing) {
              await prisma.structuralDrift.create({
                data: {
                  clubId: this.clubId,
            ...
            if (existing) {
              ...
              await prisma.fileMeta.create({
                data: {
                  ...
                  placementStatus: PlacementStatus.UNSORTED,
                  isUnmanaged: true,
                  driveLastModified: file.modifiedTime ? new Date(file.modifiedTime) : undefined,
                },
              })
  ...
  async getDriftItems() {
    return prisma.structuralDrift.findMany({
  ...
  async resolveDrift(driftId: string, resolution: 'ACCEPTED' | 'IGNORED'): Promise<void> {
    const drift = await prisma.structuralDrift.findUnique({ where: { id: driftId } })
    if (!drift || drift.clubId !== this.clubId) throw new Error('Drift item not found')
    ...
    await prisma.structuralDrift.update({
      where: { id: driftId },
